...
(function (root) {
  ...
  function sentimentIcon(sentiment) {
    ...
    return el('span', { class: 'qx-ni-sentiment ' + s.cls, role: 'img', 'aria-label': s.label, title: s.label, text: s.glyph });
  }
  ...
  function render(props) {
    ...
    if (r.showBadgeRow) {
      ...
      if (r.showSymbol && props.symbol) {
        ...
        badges.push(el('span', { class: cls }, kids));
      }
      if (r.showSentiment) badges.push(sentimentIcon(props.sentiment || 'neutral'));
      if (r.showAI) badges.push(aiIcon(props.aiLabel));
      if (r.showTopic && props.topic) badges.push(el('span', { class: 'qx-ni-topic', text: props.topic }));
    ...
    // DOM order: title → description → badges → meta (headline is the a11y name)
    const bodyKids = [titleEl, descEl, badgeRow, metaRow];
    if (isLocked) bodyKids.push(el('span', { id: 'qx-ni-lock-desc', class: 'qx-sr-only', text: 'This story requires an upgrade to read.' }));
    const body = el('div', { class: 'qx-ni-body' }, bodyKids.filter(Boolean));
